...
const Foods = () => {
  ...
  const filteorangeItems = products.filter((food) => {
   ...
   const displayPage = filteorangeItems.slice( pageVisted , pageVisted + productsPerPage  )
  
  const pageCount = Math.ceil(filteorangeItems.length / productsPerPage )
    ...
    return (
       <div className='flex items-center justify-center' key={id}>
         ...
         <motion.div
          ...
          key={id}
          className="rounded-lg flex border border-orange-200 flex-col w-[180px] lg:w-[200px] h-[250px]  justify-around items-center relative hover:bg-gradient-to-r hover:from-slate-50 hover:to-orange-200 hover:border-orange-400 hover:border-dotted shadow-xl cursor-pointer">
       <span class="animate-ping absolute  h-4 w-4 top-0 right-0  z-10 rounded-full bg-orange-400 opacity-75 mb-8"></span>
            <motion.img 
...
{price.toLocaleString()}</p> {cartItems && !cartItems.some((item) => item.id === id ) ? (<span onClick={() => {
         ...
         <div className='absolute top-0 text-slate-900 right-0 bg-gradient-to-br from-slate-50 to-orange-200 font-semibold rounded-tr-lg rounded-bl-lg text-xs overflow-hidden p-1 flex '>
         { [...Array(5)].map((_ , i) => (
