          ...
          {cartItems.length < 1 ? (
            ...
            <div>{cartItems.map(({id, quantity , price , image , title ,category}) => {
              return (
                ...
                <div className="w-2/6 sansPro flex flex-col justify-between "><p className="sansPro">{quantity} x <span>&#8377;</span>
{price.toLocaleString()}</p> <p className="underline underline-offset-2 sansPro py-2 text-slate-600" onClick={() => { removeFromCart({id})} }>
                Remove
               </p></div>
...
0</p>
       ...
       <select name="delivery" className="w-full py-2 px-1 bg-gray-200 outline-slate-300 font-semibold rounded-sm sansPro" onChange={ handleFee } value={ fee }>
        <option className='sansPro  w-full' value="800">STANDARD DELIVERY  <span>&#8377;</span>
800</option>
        <option className='sansPro w-full' value="1500">EXPRESS DELIVERY  <span>&#8377;</span>
1,500</option>
        <option className='sansPro w-full' value="2500">⚡SWIFT-HIGH PRIORITY  <span>&#8377;</span>
2,500</option>
      </select>
       </div>
       ...
       <div className="flex justify-between sansPro semibold my-3">
            ...
            <p className="sansPro ">{ total === 0 ? (<p  className="sansPro"><span>&#8377;</span>
0.000</p>) : (<p  className="sansPro"><span>&#8377;</span>
{subTotal().toLocaleString()}</p>)  }</p>
        </div>
