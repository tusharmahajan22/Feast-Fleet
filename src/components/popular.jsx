   ...
   const {removeFromCart, addToCart , state:{ cartItems }   } = useCartsContext()
    ...
    useEffect(() => {
      ...
      if(category === 'BURGER'){
         ...
         const filteorangeProducts = products.filter((item) => {
            return item.category === 'Burger'
         })
         setallFoods(filteorangeProducts)
      }
  ...
  return (
    <div className=' p-5 sm:p-10 lg:px-28' id='popular'>
        <p className='md:text-[32px] text-[28px] font-bold text-center'>Popular Dishes with Discount</p>
         ...
         <div className=' bg-orange-400 rounded-lg py-3 flex justify-center text-slate-900 items-center text-xs md:text-sm font-semibold md:gap-x-3'>
             ...
             return (
                <div className='flex items-center justify-center' key={id}>
                  ...
                  <motion.div
                   ...
                   key={id}
                   className="rounded-lg flex border border-orange-200 flex-col w-[180px] h-[250px]  justify-around items-center relative hover:bg-gradient-to-r  hover:from-slate-50 hover:to-orange-200 hover:border-orange-400 hover:border-dotted shadow-xl cursor-pointer">
                <span class="animate-ping absolute  h-4 w-4 top-0 right-0  z-10 rounded-full bg-orange-400 opacity-75"></span>
                     <motion.img 
                     ...
                     <div className='flex text-sm text-center py-2 font-bold justify-between items-center w-full p-2 '><p className=' p-1 rounded-lg text-slate-800'><span>&#8377;</span>
{price.toLocaleString()}</p> {cartItems && !cartItems.some((item) => item.id === id ) ? (<span onClick={() => {
                        addItem({ id , price , title ,image01 , quantity ,category})
                     }} className='text-lg px-2 rounded text-white bg-orange-300  hover:bg-white hover:text-orange-400' 
                     >+</span>)  :(<span disabled onClick={remove} className='text-lg px-2 rounded  text-white bg-orange-300  hover:bg-white hover:text-orange-400' 
                     ><TiDelete className='w-6 h-6' /></span>)} 
                     </div>
                  <div className='absolute top-0 text-slate-900 right-0 bg-gradient-to-br from-slate-50 to-orange-300 font-semibold rounded-tr-lg rounded-bl-lg text-xs overflow-hidden p-1'>10% OFF</div>
                 </motion.div>
                </div>
             )
          }).slice(0,6)}
          ...
          <div className='flex justify-center'>
            <Link to='/foods'>
               <span className=' cursor-pointer border-2 border-orange-400 shadow-xl rounded-full p-1 gap-x-2 bg-orange-400 text-xs px-2 text-white flex justify-center items-center font-semibold '>See all foods 
               <div className="bg-white text-orange-400 font-bold rounded-full p-[2px] text-lg ">
